...
export const discordCallback = (code, state, error, callbackUrl = "") => {
    return axiosInstance.post(`/api/aria/v1/bind/discord${'?callbackUrl=' + callbackUrl}`, { code, state, error }).then(ret => ret.data);
}
//get social media bind url
export const socialMediaAuthorize = (type, callbackUrl = "") => {
    return axiosInstance.get(`/api/activity/v1/socialMedia/authorize/${type || 'discord'}${'?callbackUrl=' + callbackUrl} `).then(ret => ret.data);
}


// social media call back
export const socialMediaCallBack = (type, oauth_token, oauth_verifier, callbackUrl = "") => {
    return axiosInstance.post(`/api/activity/v1/socialMedia/call_back/${type}?${'callbackUrl=' + callbackUrl} `, { oauth_token, oauth_verifier }).then(ret => ret.data);
}
